...
- **`POST /api/save/email/send`** — `{ email, data }` (`data.characterId`
  already carries the character's identity, same as the existing
  code-transfer route's `{ data }`-only shape — no separate field
  needed). The *only* route that can trigger an email. Generates a
  fresh code, writes `emailcode:<code>` to KV (24h `expirationTtl`),
  emails the code to `email` via Resend, and immediately discards the
  `email` variable — never written to KV, never logged (see Privacy).
  **Returns `{ ok, code }`** — the originating device gets its own copy
  of the code back synchronously (it doesn't need to check its own
  email) so it can keep auto-pushing to it (see Client flow).
- **`POST /api/save/email/push`** — `{ code, data }`.
...
- **`POST /api/save/email/redeem`** — `{ code }`. Returns the stored
  `{ data }` and **deletes the key** — one-shot, like the
...
8 characters from a 31-symbol alphabet excluding visually-similar
characters (digits `0`/`1` and letters `i`/`l`/`o` removed — 8 digits
+ 23 letters) — unlike the existing 4-char transfer code, this one
lives up to 24 hours and is worth a bigger keyspace, and avoiding
ambiguous characters matters more here since a user might be reading
it off a phone screen and typing it into a different device's keyboard
rather than copy-pasting. `31^8 ≈ 8.5 × 10^11` combinations — the real
defense is still the per-IP rate limit on `redeem` (see below), not
the alphabet size alone, same
philosophy as the existing 4-char code's own comment in
`js/systems/cloudSave.js`; this alphabet is just sized to match a
much longer exposure window. Displayed and entered as a plain 8-character
string (no grouping/hyphens) — kept simple rather than adding display
grouping plus the input-side un-grouping it would require.
...
- `emailcode:<code>` → `{ data, savedAt }`. Written by
...
Resend also requires the sending domain to be verified via DNS records
(added in Resend's dashboard) before it will actually deliver mail from
`rpg@rpg.burghertime.com` (`functions/_shared/sendEmail.js`'s
`FROM_ADDRESS`) — an unverified domain's sends are rejected outright.
This is part of the same Resend account setup as the API key above, not
a separate step, but is called out explicitly here because it's easy to
set the two secrets and assume the feature is ready when it isn't.
